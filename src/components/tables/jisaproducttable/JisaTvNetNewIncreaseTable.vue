...
export default {
  ...
  name:'JisaTvNetNewIncreaseTable',
  extends: Line,
  mixins: [reactiveProp],
  ...
  async created () {
    await axios.get(jisaNetNewIncreaseUrl+'북부고객본부').then((res)=>{
      ...
      //console.log('xxx is ', this.bonbuNetIncreaseData);
  ...
  methods: {
    ...
    getDesserts (bonbuName) {
      ...
      console.log('yyy.fifthJisa is ', yyy.fifthJisa.sysdate,yyy.fifthJisa.countSum);
      const dessertsArrsy=new Array();
      ...
      if(bonbuName==='북부고객본부'){
        for(let i=yyy.firstJisa.sysdate.length-1;i>0;i--){
          ...
          dessertsArrsy.push(obj);
        }
        this.desserts=Object.values(dessertsArrsy);
    ...
    getBonbuNetIncreaseValue(url){    //본부별 순익(y축)
      ...
      //console.log('fifthProductArray is', fifthSysdateArray);
