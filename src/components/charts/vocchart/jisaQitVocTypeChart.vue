  ...
  methods: {
    ...
    getBonbuVocValue(selectedBonbu){    
      ...
      this.bonbuVocData.map((item,index)=>{
        // console.log('voc1 is ',item.voc_gubun);
        if(this.propsvoctype==="부주의,ID/PW분실등") this.propsvoctype='\"부주의,ID/PW분실등\"';
        if(item.voc_gubun.replace(/ /g,'')===this.propsvoctype){
        // if(this.propstype==='접속불가'){
